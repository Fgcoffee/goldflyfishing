...
def keyword_entries(rules) -> dict[str, Entry]:
    ...
    """
    from . import keywords as keyword_registry
    ...
    for rule in rules:
        ...
        if spec is None:
            ...
            out[rule.number] = Entry(Status.IMPLEMENTED, "rules/keywords.py", name)
        elif spec.status is keyword_registry.Status.PARTIAL:
            out[rule.number] = Entry(Status.PARTIAL, "rules/keywords.py", f"{name}: {spec.note}")
        else:
