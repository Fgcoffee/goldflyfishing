...
from ..cr600_spells_and_abilities.cr603_triggers import put_triggers_on_stack
from ..cr600_spells_and_abilities.cr608_stack import resolve_top
from ..cr700_additional_rules.cr704_sba import check_state_based_actions
from ..kernel.enums import NO_PRIORITY_STEPS
...
#: a free ability, a loop with no cost - can reach it. See rules.cr500_turn_structure.cr500_turn.
...
def settle(game: Game) -> None:
    ...
    """
    from ..cr600_spells_and_abilities.cr603_triggers import check_state_triggers
...
def run_priority(game: Game) -> None:
    ...
    from ..kernel.loops import action_step, begin_window, note_step, resolution_step
    ...
    for _ in range(MAX_PRIORITY_ROUNDS):
        ...
        # through it. See rules.cr500_turn_structure.cr500_turn.ACTION_BUDGET.
...
def _decide(game: Game, player_id: PlayerId) -> Action:
    ...
    if agent is None:
        return PASS
    from ..kernel.legality import legal_actions
    ...
    if game.exhausted_loop_steps:
        ...
        from ..kernel.loops import is_exhausted
